...
void useComputer() {
    ...
    if ( queryCard(name) != NULL) {
        ...
        if ( strcmp(queryCard(name)->aPwd, pwd) == 0 ) {   //判断密码是否正确

            if ( queryCard(name)->fBalance > 0 ) {       //判断卡内是否还有余额

                if ( queryCard(name)->nStatus == 0 ) {   //判断卡的状态是否处于未上机状态

                    queryCard(name)->tLast = time(NULL);  //修改最后一次使用此卡的时间为当前时间
                    queryCard(name)->nStatus = 1;        //将卡的状态改为正在上机
                    queryCard(name)->nUseCount++;        //该卡的使用次数加一
                    printf("上机成功！\n");
        ...
        if ( strcmp(p->aPwd, pwd) == 0 ) {  //判断密码是否正确
            ...
            if ( p->nStatus == 1 ) {        //判断卡是否处于上机状态
                if(!p->isVip) {
                    ...
                    second = time(NULL) - p->tLast;  //用当前时间减去上次使用卡系统的时间 计算时间差，为此次上机总时间
                    money = (second / 3600 + !(!(second % 3600))) * VIP_price; //vip计算费用，不足一个小时按一个小时算
                    p->fTotalUse += money;
                    printf("花费了%.2lf元。\n", money);
                }
                while ( true ) {
...
void addVip(){
    ...
    if ( queryCard(name)) {//判断想要添加的卡号是否已经存在
        if ( queryCard(name)->nStatus != 2 ) {
            ...
            printf("成为VIP需要扣费100,是否继续?(1为继续，0为退出): ");
            int y = 0;
            scanf("%d",&y);
            if(y == 1){
